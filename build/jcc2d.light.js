...
var FN_MAPS$1 = {
  loopIn: function loopIn(datak, mode, offset) {
    ...
    switch (mode) {
      case 'cycle':
        return new Cycle$1('in', begin, end);
      case 'pingpong':
        return new Pingpong$1('in', begin, end);
      default:
        break;
    }
    return null;
  ...
  loopOut: function loopOut(datak, mode, offset) {
    ...
    switch (mode) {
      case 'cycle':
        return new Cycle$1('out', begin, end);
      case 'pingpong':
        return new Pingpong$1('out', begin, end);
      default:
        break;
    }
    return null;
 ...
 * @return {array}
 */
function parseParams$1(pStr) {
  var params = pStr.split(/\s*,\s*/);
  return params.map(function (it) {
    if (STR_REG$1.test(it)) return it.replace(/"|'/g, '');
    return parseInt(it);
  });
}
 ...
 * @return {object}
 */
function parseEx$1(ex) {
  var rs = ex.match(EX_REG$1);
  var ps = parseParams$1(rs[2]);
  return {
    name: rs[1],
    mode: ps[0],
    offset: ps[1]
 ...
 */
function getEX(ksp) {
  var _parseEx = parseEx$1(ksp.x),
      name = _parseEx.name,
      mode = _parseEx.mode,
      offset = _parseEx.offset;

  var _offset = offset === 0 ? ksp.k.length - 1 : offset;
  return FN_MAPS$1[name] && FN_MAPS$1[name](ksp.k, mode, _offset);
}
...
var NullElement$2 = function (_Container) {
 ...
 * @return {*}
 */
function createRegularArray$1(type, len) {
  var i = 0;
  var arr = [];
  var value = void 0;
  switch (type) {
 ...
 * @return {*}
 */
function _createTypedArray$2(type, len) {
  if (type === 'float32') {
    return new Float32Array(len);
  } else if (type === 'int16') {
    return new Int16Array(len);
...
var createTypedArray$1 = void 0;
//  = createTypedArray
if (typeof Uint8ClampedArray === 'function' && typeof Float32Array === 'function') {
  createTypedArray$1 = _createTypedArray$2;
} else {
  createTypedArray$1 = createRegularArray$1;
}
...
var Matrix$3 = function () {
    ...
    function applyToTriplePoints(pt1, pt2, pt3) {
        var arr = createTypedArray$1('float32', 6);
        if (this.isIdentity()) {
            arr[0] = pt1[0];
            arr[1] = pt1[1];
            arr[2] = pt2[0];
    ...
    return function () {
        ...
        this.props = createTypedArray$1('float32', 16);
        this.reset();
    };
}();
...
var ShapeTransformManager$2 = function () {
  /**
   * a
   */
  function ShapeTransformManager() {
 ...
 * @return {*}
 */
function double$1(arr) {
  return arr.concat(createSizedArray$1(arr.length));
}

var pooling$1 = { double: double$1 };
...
var defaultCurveSegments$3 = 200;
/**
 * a
 * @return {*}
 */
function create$4() {
  return {
    addedLength: 0,
    percents: createTypedArray$1('float32', defaultCurveSegments$3),
    lengths: createTypedArray$1('float32', defaultCurveSegments$3)
  };
}
var bezier_length_pool = pool_factory(8, create$4);
...
var segments_length_pool = pool_factory(8, create$5, release$3);

// var easingFunctions = [];
// var math = Math;
var defaultCurveSegments$2 = 200;
 ...
 * @return {*}
 */
function pointOnLine2D$1(x1, y1, x2, y2, x3, y3) {
  var det1 = x1 * y2 + y1 * x3 + x2 * y3 - x3 * y2 - y3 * x1 - x2 * y1;
  return det1 > -0.001 && det1 < 0.001;
}
 ...
 * @return {*}
 */
function pointOnLine3D$1(x1, y1, z1, x2, y2, z2, x3, y3, z3) {
  if (z1 === 0 && z2 === 0 && z3 === 0) {
    return pointOnLine2D$1(x1, y1, x2, y2, x3, y3);
  }
  var dist1 = Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2) + Math.pow(z2 - z1, 2));
  var dist2 = Math.sqrt(Math.pow(x3 - x1, 2) + Math.pow(y3 - y1, 2) + Math.pow(z3 - z1, 2));
  var dist3 = Math.sqrt(Math.pow(x3 - x2, 2) + Math.pow(y3 - y2, 2) + Math.pow(z3 - z2, 2));
 ...
 * @return {*}
 */
function getBezierLength$1(pt1, pt2, pt3, pt4) {
  var curveSegments = defaultCurveSegments$2 || 200;
  // var i, len;
  var addedLength = 0;
  var ptDistance = void 0;
  var point = [];
 ...
 * @return {*}
 */
function getSegmentsLength$1(shapeData) {
  ...
  var i = 0;
  for (; i < len - 1; i += 1) {
    lengths[i] = getBezierLength$1(pathV[i], pathV[i + 1], pathO[i], pathI[i + 1]);
    totalLength += lengths[i].addedLength;
  }
  if (closed && len) {
    lengths[i] = getBezierLength$1(pathV[i], pathV[0], pathO[i], pathI[0]);
    totalLength += lengths[i].addedLength;
  }
  segmentsLength.totalLength = totalLength;
  return segmentsLength;
 ...
 * @param {*} point a
 */
function PointData$1(partial, point) {
  this.partialLength = partial;
  this.point = point;
}
 ...
 * @return {*}
 */
function buildBezierData$1(pt1, pt2, pt3, pt4) {
  var bezierName = (pt1[0] + '_' + pt1[1] + '_' + pt2[0] + '_' + pt2[1] + '_' + pt3[0] + '_' + pt3[1] + '_' + pt4[0] + '_' + pt4[1]).replace(/\./g, 'p');
  if (!storedData$1[bezierName]) {
    var curveSegments = defaultCurveSegments$2;
    // var k, i, len;
    var addedLength = 0;
    var ptDistance = void 0;
    var point = void 0;
    var lastPoint = null;
    if (pt1.length === 2 && (pt1[0] != pt2[0] || pt1[1] != pt2[1]) && pointOnLine2D$1(pt1[0], pt1[1], pt2[0], pt2[1], pt1[0] + pt3[0], pt1[1] + pt3[1]) && pointOnLine2D$1(pt1[0], pt1[1], pt2[0], pt2[1], pt2[0] + pt4[0], pt2[1] + pt4[1])) {
      curveSegments = 2;
    }
    var bezierData = new BezierData$1(curveSegments);
    var len = pt3.length;
    for (var k = 0; k < curveSegments; k += 1) {
      ...
      ptDistance = Math.sqrt(ptDistance);
      addedLength += ptDistance;
      bezierData.points[k] = new PointData$1(ptDistance, point);
      lastPoint = point;
    }
    bezierData.segmentLength = addedLength;
    storedData$1[bezierName] = bezierData;
  }
  return storedData$1[bezierName];
}
 ...
 * @return {*}
 */
function getDistancePerc$1(perc, bezierData) {
  var percents = bezierData.percents;
  var lengths = bezierData.lengths;
  var len = percents.length;
  var initPos = Math.floor((len - 1) * perc);
 ...
 * @return {*}
 */
function getPointInSegment$1(pt1, pt2, pt3, pt4, percent, bezierData) {
  var t1 = getDistancePerc$1(percent, bezierData);
  // var u0 = 1;
  var u1 = 1 - t1;
  var ptX = Math.round((u1 * u1 * u1 * pt1[0] + (t1 * u1 * u1 + u1 * t1 * u1 + u1 * u1 * t1) * pt3[0] + (t1 * t1 * u1 + u1 * t1 * t1 + t1 * u1 * t1) * pt4[0] + t1 * t1 * t1 * pt2[0]) * 1000) / 1000;
  var ptY = Math.round((u1 * u1 * u1 * pt1[1] + (t1 * u1 * u1 + u1 * t1 * u1 + u1 * u1 * t1) * pt3[1] + (t1 * t1 * u1 + u1 * t1 * t1 + t1 * u1 * t1) * pt4[1] + t1 * t1 * t1 * pt2[1]) * 1000) / 1000;
...
var bezierSegmentPoints$1 = createTypedArray$1('float32', 8);
 ...
 * @return {*}
 */
function getNewSegment$1(pt1, pt2, pt3, pt4, startPerc, endPerc, bezierData) {
  startPerc = startPerc < 0 ? 0 : startPerc > 1 ? 1 : startPerc;
  var t0 = getDistancePerc$1(startPerc, bezierData);
  endPerc = endPerc > 1 ? 1 : endPerc;
  var t1 = getDistancePerc$1(endPerc, bezierData);
  var len = pt1.length;
  var u0 = 1 - t0;
  var u1 = 1 - t1;
  var u0u0u0 = u0 * u0 * u0;
  ...
  var t1t1t1 = t1 * t1 * t1;
  for (var i = 0; i < len; i += 1) {
    bezierSegmentPoints$1[i * 4] = Math.round((u0u0u0 * pt1[i] + t0u0u0_3 * pt3[i] + t0t0u0_3 * pt4[i] + t0t0t0 * pt2[i]) * 1000) / 1000;
    bezierSegmentPoints$1[i * 4 + 1] = Math.round((u0u0u1 * pt1[i] + t0u0u1_3 * pt3[i] + t0t0u1_3 * pt4[i] + t0t0t1 * pt2[i]) * 1000) / 1000;
    bezierSegmentPoints$1[i * 4 + 2] = Math.round((u0u1u1 * pt1[i] + t0u1u1_3 * pt3[i] + t0t1u1_3 * pt4[i] + t0t1t1 * pt2[i]) * 1000) / 1000;
    bezierSegmentPoints$1[i * 4 + 3] = Math.round((u1u1u1 * pt1[i] + t1u1u1_3 * pt3[i] + t1t1u1_3 * pt4[i] + t1t1t1 * pt2[i]) * 1000) / 1000;
  }

  return bezierSegmentPoints$1;
}

var bez$1 = {
  getSegmentsLength: getSegmentsLength$1,
  getNewSegment: getNewSegment$1,
  getPointInSegment: getPointInSegment$1,
  buildBezierData: buildBezierData$1,
  pointOnLine2D: pointOnLine2D$1,
  pointOnLine3D: pointOnLine3D$1
};
 ...
 * @param {*} quat a
 */
function quaternionToEuler$1(out, quat) {
  var qx = quat[0];
  var qy = quat[1];
  var qz = quat[2];
  var qw = quat[3];
  ...
  var attitude = Math.asin(2 * qx * qy + 2 * qz * qw);
  var bank = Math.atan2(2 * qx * qw - 2 * qy * qz, 1 - 2 * qx * qx - 2 * qz * qz);
  out[0] = heading / degToRads$2;
  out[1] = attitude / degToRads$2;
  out[2] = bank / degToRads$2;
}
 ...
 * @return {*}
 */
function createQuaternion$1(values) {
  var heading = values[0] * degToRads$2;
  var attitude = values[1] * degToRads$2;
  var bank = values[2] * degToRads$2;
  var c1 = Math.cos(heading / 2);
  var c2 = Math.cos(attitude / 2);
  var c3 = Math.cos(bank / 2);
  var s1 = Math.sin(heading / 2);
...
var BaseProperty$1 = function () {
  ...
  createClass(BaseProperty, [{
    ...
    value: function interpolateValue(frameNum, caching) {
      ...
      var newValue = void 0;
      if (this.propType === 'multidimensional') {
        newValue = createTypedArray$1('float32', this.pv.length);
      }
      var iterationIndex = caching.lastIndex;
      var i = iterationIndex;
      var len = this.keyframes.length - 1;
      ...
      if (keyData.to) {
        if (!keyData.bezierData) {
          keyData.bezierData = bez$1.buildBezierData(keyData.s, nextKeyData.s || keyData.e, keyData.to, keyData.ti);
        }
        var bezierData = keyData.bezierData;
        if (frameNum >= nextKeyTime || frameNum < keyTime) {
          var ind = frameNum >= nextKeyTime ? bezierData.points.length - 1 : 0;
          ...
          if (keyData.__fnct) {
            ...
            fnc = BezierFactory$1.getBezierEasing(keyData.o.x, keyData.o.y, keyData.i.x, keyData.i.y, keyData.n).get;
            keyData.__fnct = fnc;
          }
          perc = fnc((frameNum - keyTime) / (nextKeyTime - keyTime));
          var distanceInLine = bezierData.segmentLength * perc;
        ...
        if (this.sh && keyData.h !== 1) {
          if (frameNum >= nextKeyTime) {
            ...
            var quatStart = createQuaternion$1(keyData.s);
            var quatEnd = createQuaternion$1(endValue);
            var time = (frameNum - keyTime) / (nextKeyTime - keyTime);
            quaternionToEuler$1(newValue, slerp$1(quatStart, quatEnd, time));
          }
        } else {
          for (i = 0; i < _len; i += 1) {
            if (keyData.h !== 1) {
              if (frameNum >= nextKeyTime) {
                ...
                if (keyData.o.x.constructor === Array) {
                  ...
                  if (!keyData.__fnct[i]) {
                    ...
                    inX = typeof keyData.i.x[i] === 'undefined' ? keyData.i.x[0] : keyData.i.x[i];
                    inY = typeof keyData.i.y[i] === 'undefined' ? keyData.i.y[0] : keyData.i.y[i];
                    fnc = BezierFactory$1.getBezierEasing(outX, outY, inX, inY).get;
                    keyData.__fnct[i] = fnc;
                  } else {
                    fnc = keyData.__fnct[i];
                  }
...
var ValueProperty$1 = function (_BaseProperty) {
  inherits(ValueProperty, _BaseProperty);
...
var MultiDimensionalProperty$1 = function (_BaseProperty2) {
  inherits(MultiDimensionalProperty, _BaseProperty2);
  ...
  function MultiDimensionalProperty(elem, data, mult, container) {
    ...
    _this2.kf = false;
    var len = data.k.length;
    _this2.v = createTypedArray$1('float32', len);
    _this2.pv = createTypedArray$1('float32', len);
    // var arr = createTypedArray('float32', len);
    _this2.vel = createTypedArray$1('float32', len);
    for (var i = 0; i < len; i += 1) {
      _this2.v[i] = data.k[i] * _this2.mult;
      _this2.pv[i] = data.k[i];
    }
...
var KeyframedValueProperty$1 = function (_BaseProperty3) {
  inherits(KeyframedValueProperty, _BaseProperty3);
...
var KeyframedMultidimensionalProperty$1 = function (_BaseProperty4) {
  inherits(KeyframedMultidimensionalProperty, _BaseProperty4);
  ...
  function KeyframedMultidimensionalProperty(elem, data, mult, container) {
    ...
    for (var i = 0; i < len - 1; i += 1) {
      if (data.k[i].to && data.k[i].s && data.k[i].e) {
        ...
        to = data.k[i].to;
        ti = data.k[i].ti;
        if (s.length === 2 && !(s[0] === e[0] && s[1] === e[1]) && bez$1.pointOnLine2D(s[0], s[1], e[0], e[1], s[0] + to[0], s[1] + to[1]) && bez$1.pointOnLine2D(s[0], s[1], e[0], e[1], e[0] + ti[0], e[1] + ti[1]) || s.length === 3 && !(s[0] === e[0] && s[1] === e[1] && s[2] === e[2]) && bez$1.pointOnLine3D(s[0], s[1], s[2], e[0], e[1], e[2], s[0] + to[0], s[1] + to[1], s[2] + to[2]) && bez$1.pointOnLine3D(s[0], s[1], s[2], e[0], e[1], e[2], e[0] + ti[0], e[1] + ti[1], e[2] + ti[2])) {
          data.k[i].to = null;
          data.k[i].ti = null;
        }
        if (s[0] === e[0] && s[1] === e[1] && to[0] === 0 && to[1] === 0 && ti[0] === 0 && ti[1] === 0) {
    ...
    _this4.getValue = _this4.processEffectsSequence;
    var arrLen = data.k[0].s.length;
    _this4.v = createTypedArray$1('float32', arrLen);
    _this4.pv = createTypedArray$1('float32', arrLen);
    ...
    _this4._caching = {
      lastFrame: initFrame,
      lastIndex: 0,
      value: createTypedArray$1('float32', arrLen)
    };
    return _this4;
  }
...
function PropertyFactory$1(elem, data, type, mult, container) {
  var p = void 0;
  if (!data.k.length) {
    p = new ValueProperty$1(elem, data, mult, container);
  } else if (typeof data.k[0] === 'number') {
    p = new MultiDimensionalProperty$1(elem, data, mult, container);
  } else {
    switch (type) {
      case 0:
        p = new KeyframedValueProperty$1(elem, data, mult, container);
        break;
      case 1:
        p = new KeyframedMultidimensionalProperty$1(elem, data, mult, container);
        break;
    }
  }
  if (p.effectsSequence.length) {
 ...
 */
function create$7() {
  return createTypedArray$1('float32', 2);
}
var point_pool = pool_factory(8, create$7);

/**
 * a
 */

var ShapePath$2 = function () {
  /**
   * a
   */
  function ShapePath() {
...
var shape_pool = pool_factory(4, create$6, release$4);
shape_pool.clone = clone$1;

/**
 * a
 */

var ShapeCollection$2 = function () {
  /**
   * a
   */
  function ShapeCollection() {
...
var _length$1 = 0;
var _maxLength$1 = 4;
var pool$1 = createSizedArray$1(_maxLength$1);

/**
 * a
 * @return {*}
 */
function newShapeCollection$1() {
  var shapeCollection = void 0;
  if (_length$1) {
    _length$1 -= 1;
    shapeCollection = pool$1[_length$1];
  } else {
    shapeCollection = new ShapeCollection$2();
  }
  return shapeCollection;
}
 ...
 * @param {*} shapeCollection a
 */
function release$5(shapeCollection) {
  var len = shapeCollection._length;
  for (var i = 0; i < len; i += 1) {
  ...
  if (_length$1 === _maxLength$1) {
    pool$1 = pooling$1.double(pool$1);
    _maxLength$1 = _maxLength$1 * 2;
  }
  pool$1[_length$1] = shapeCollection;
  _length$1 += 1;
}

var shapeCollection_pool = { newShapeCollection: newShapeCollection$1, release: release$5 };

/**
 * a
 */
var DynamicPropertyContainer$2 = function () {
  function DynamicPropertyContainer() {
    classCallCheck(this, DynamicPropertyContainer);
  }
...
var BaseShapeProperty$1 = function () {
  function BaseShapeProperty() {
    classCallCheck(this, BaseShapeProperty);
  }
        ...
        if (!isHold) {
          if (frameNum >= nextKeyData.t - this.offsetTime) {
            ...
            if (keyData.__fnct) {
              ...
              fnc = BezierFactory$1.getBezierEasing(keyData.o.x, keyData.o.y, keyData.i.x, keyData.i.y).get;
              keyData.__fnct = fnc;
            }
...
var ShapeProperty$1 = function (_BaseShapeProperty) {
  inherits(ShapeProperty, _BaseShapeProperty);
...
var KeyframedShapeProperty$1 = function (_BaseShapeProperty2) {
  inherits(KeyframedShapeProperty, _BaseShapeProperty2);
...
var roundCorner$2 = 0.5519;
var cPoint$1 = roundCorner$2;

/**
 * a
 */

var EllShapeProperty$1 = function (_DynamicPropertyConta) {
  inherits(EllShapeProperty, _DynamicPropertyConta);
  ...
  function EllShapeProperty(elem, data) {
    ...
    _this3.elem = elem;
    _this3.initDynamicPropertyContainer(elem);
    _this3.p = PropertyFactory$1(elem, data.p, 1, 0, _this3);
    _this3.s = PropertyFactory$1(elem, data.s, 1, 0, _this3);
    if (_this3.dynamicProperties.length) {
      _this3.k = true;
    } else {
      _this3.k = false;
  ...
  createClass(EllShapeProperty, [{
    ...
    value: function convertEllToPath() {
      ...
      _v.v[3][0] = _cw ? p0 - s0 : p0 + s0;
      _v.v[3][1] = p1;
      _v.i[0][0] = _cw ? p0 - s0 * cPoint$1 : p0 + s0 * cPoint$1;
      _v.i[0][1] = p1 - s1;
      _v.i[1][0] = _cw ? p0 + s0 : p0 - s0;
      _v.i[1][1] = p1 - s1 * cPoint$1;
      _v.i[2][0] = _cw ? p0 + s0 * cPoint$1 : p0 - s0 * cPoint$1;
      _v.i[2][1] = p1 + s1;
      _v.i[3][0] = _cw ? p0 - s0 : p0 + s0;
      _v.i[3][1] = p1 + s1 * cPoint$1;
      _v.o[0][0] = _cw ? p0 + s0 * cPoint$1 : p0 - s0 * cPoint$1;
      _v.o[0][1] = p1 - s1;
      _v.o[1][0] = _cw ? p0 + s0 : p0 - s0;
      _v.o[1][1] = p1 + s1 * cPoint$1;
      _v.o[2][0] = _cw ? p0 - s0 * cPoint$1 : p0 + s0 * cPoint$1;
      _v.o[2][1] = p1 + s1;
      _v.o[3][0] = _cw ? p0 - s0 : p0 + s0;
      _v.o[3][1] = p1 - s1 * cPoint$1;
    }
  }]);
  return EllShapeProperty;
}(DynamicPropertyContainer$2);
...
var StarShapeProperty$1 = function (_DynamicPropertyConta2) {
  inherits(StarShapeProperty, _DynamicPropertyConta2);
  ...
  function StarShapeProperty(elem, data) {
    ...
    _this4.initDynamicPropertyContainer(elem);
    if (data.sy === 1) {
      _this4.ir = PropertyFactory$1(elem, data.ir, 0, 0, _this4);
      _this4.is = PropertyFactory$1(elem, data.is, 0, 0.01, _this4);
      _this4.convertToPath = _this4.convertStarToPath;
    } else {
      _this4.convertToPath = _this4.convertPolygonToPath;
    }
    _this4.pt = PropertyFactory$1(elem, data.pt, 0, 0, _this4);
    _this4.p = PropertyFactory$1(elem, data.p, 1, 0, _this4);
    _this4.r = PropertyFactory$1(elem, data.r, 0, degToRads$3, _this4);
    _this4.or = PropertyFactory$1(elem, data.or, 0, 0, _this4);
    _this4.os = PropertyFactory$1(elem, data.os, 0, 0.01, _this4);
...
var RectShapeProperty$1 = function (_DynamicPropertyConta3) {
  inherits(RectShapeProperty, _DynamicPropertyConta3);
  ...
  function RectShapeProperty(elem, data) {
    ...
    _this5.d = data.d;
    _this5.initDynamicPropertyContainer(elem);
    _this5.p = PropertyFactory$1(elem, data.p, 1, 0, _this5);
    _this5.s = PropertyFactory$1(elem, data.s, 1, 0, _this5);
    _this5.r = PropertyFactory$1(elem, data.r, 0, 0, _this5);
    if (_this5.dynamicProperties.length) {
      _this5.k = true;
    } else {
      _this5.k = false;
  ...
  createClass(RectShapeProperty, [{
    ...
    value: function convertRectToPath() {
      ...
      var v1 = this.s.v[1] / 2;
      var round = Math.min(v0, v1, this.r.v);
      var cPoint = round * (1 - roundCorner$2);
      this.v._length = 0;
...
function getShapeProp$1(elem, data, type) {
  var prop = void 0;
  if (type === 3 || type === 4) {
    var dataProp = type === 3 ? data.pt : data.ks;
    var keys = dataProp.k;
    if (keys.length) {
      prop = new KeyframedShapeProperty$1(elem, data, type);
    } else {
      prop = new ShapeProperty$1(elem, data, type);
    }
  } else if (type === 5) {
    prop = new RectShapeProperty$1(elem, data);
  } else if (type === 6) {
    prop = new EllShapeProperty$1(elem, data);
  } else if (type === 7) {
    prop = new StarShapeProperty$1(elem, data);
  }
  if (prop.k) {
    elem.addDynamicProperty(prop);
  }
 ...
 * @return {*}
 */
function getConstructorFunction$1() {
  return ShapeProperty$1;
}
 ...
 * @return {*}
 */
function getKeyframedConstructorFunction$1() {
  return KeyframedShapeProperty$1;
}

var ShapePropertyFactory$1 = { getShapeProp: getShapeProp$1, getConstructorFunction: getConstructorFunction$1, getKeyframedConstructorFunction: getKeyframedConstructorFunction$1 };
...
var TransformProperty$1 = function (_DynamicPropertyConta) {
  inherits(TransformProperty, _DynamicPropertyConta);
  ...
  function TransformProperty(elem, data, container) {
    ...
    _this.propType = 'transform';
    _this.data = data;
    _this.v = new Matrix$3();
    // Precalculated matrix with non animated properties
    _this.pre = new Matrix$3();
    _this.appliedTransformations = 0;
    _this.initDynamicPropertyContainer(container || elem);
    if (data.p && data.p.s) {
      _this.px = PropertyFactory$1(elem, data.p.x, 0, 0, _this);
      _this.py = PropertyFactory$1(elem, data.p.y, 0, 0, _this);
      if (data.p.z) {
        _this.pz = PropertyFactory$1(elem, data.p.z, 0, 0, _this);
      }
    } else {
      _this.p = PropertyFactory$1(elem, data.p || { k: [0, 0, 0] }, 1, 0, _this);
    }
    if (data.rx) {
      _this.rx = PropertyFactory$1(elem, data.rx, 0, degToRads$4, _this);
      _this.ry = PropertyFactory$1(elem, data.ry, 0, degToRads$4, _this);
      _this.rz = PropertyFactory$1(elem, data.rz, 0, degToRads$4, _this);
      if (data.or.k[0].ti) {
      ...
      _this.or = PropertyFactory$1(elem, data.or, 1, degToRads$4, _this);
      // sh Indicates it needs to be capped between -180 and 180
      _this.or.sh = true;
    } else {
      _this.r = PropertyFactory$1(elem, data.r || { k: 0 }, 0, degToRads$4, _this);
    }
    if (data.sk) {
      _this.sk = PropertyFactory$1(elem, data.sk, 0, degToRads$4, _this);
      _this.sa = PropertyFactory$1(elem, data.sa, 0, degToRads$4, _this);
    }
    _this.a = PropertyFactory$1(elem, data.a || { k: [0, 0, 0] }, 1, 0, _this);
    _this.s = PropertyFactory$1(elem, data.s || { k: [100, 100, 100] }, 1, 0.01, _this);
    // Opacity is not part of the transform properties, that's why it won't use this.dynamicProperties. That way transforms won't get updated if opacity changes.
    if (data.o) {
      _this.o = PropertyFactory$1(elem, data.o, 0, 0.01, elem);
    } else {
      _this.o = { _mdf: false, v: 1 };
    }
    _this._isDirty = true;
...
function getTransformProperty$1(elem, data, container) {
  return new TransformProperty$1(elem, data, container);
}

var TransformPropertyFactory = { getTransformProperty: getTransformProperty$1 };

/**
 * a
 */

var ShapeModifier$2 = function (_DynamicPropertyConta) {
  inherits(ShapeModifier, _DynamicPropertyConta);
...
var TrimModifier$2 = function (_ShapeModifier) {
  inherits(TrimModifier, _ShapeModifier);
  ...
  createClass(TrimModifier, [{
     ...
     */
    value: function initModifierProperties(elem, data) {
      this.s = PropertyFactory$1(elem, data.s, 0, 0.01, this);
      this.e = PropertyFactory$1(elem, data.e, 0, 0.01, this);
      this.o = PropertyFactory$1(elem, data.o, 0, 0, this);
      this.sValue = 0;
      this.eValue = 0;
      this.getValue = this.processKeys;
      this.m = data.m;
        ...
        for (var _i = 0; _i < len; _i += 1) {
          ...
          if (!shapeData.shape._mdf && !this._mdf && !_isFirstFrame && this.m !== 2) {
            ...
            if (!shapeData.shape._mdf && shapeData.pathsData.length) {
              ...
              pathsData = this.releasePathsData(shapeData.pathsData);
              for (var j = 0; j < jLen; j += 1) {
                pathData = bez$1.getSegmentsLength(shapePaths.shapes[j]);
                pathsData.push(pathData);
                totalShapeLength += pathData.totalLength;
              }
              shapeData.totalShapeLength = totalShapeLength;
        ...
        for (; j < jLen; j += 1) {
          ...
          if (addedLength + currentLengthData.addedLength < shapeSegment.s) {
            ...
            if (shapeSegment.s <= addedLength && shapeSegment.e >= addedLength + currentLengthData.addedLength) {
              ...
              segment = bez$1.getNewSegment(shapePaths[i].v[j - 1], shapePaths[i].v[j], shapePaths[i].o[j - 1], shapePaths[i].i[j], (shapeSegment.s - addedLength) / currentLengthData.addedLength, (shapeSegment.e - addedLength) / currentLengthData.addedLength, lengths[j - 1]);
              this.addSegmentFromArray(segment, shapePath, segmentCount, newShape);
              // this.addSegment(segment.pt1, segment.pt3, segment.pt4, segment.pt2, shapePath, segmentCount, newShape);
              newShape = false;
              shapePath.c = false;
        ...
        if (shapePaths[i].c && lengths.length) {
          ...
          if (addedLength <= shapeSegment.e) {
            ...
            if (shapeSegment.s <= addedLength && shapeSegment.e >= addedLength + segmentLength) {
              ...
              segment = bez$1.getNewSegment(shapePaths[i].v[j - 1], shapePaths[i].v[0], shapePaths[i].o[j - 1], shapePaths[i].i[0], (shapeSegment.s - addedLength) / segmentLength, (shapeSegment.e - addedLength) / segmentLength, lengths[j - 1]);
              this.addSegmentFromArray(segment, shapePath, segmentCount, newShape);
              // this.addSegment(segment.pt1, segment.pt3, segment.pt4, segment.pt2, shapePath, segmentCount, newShape);
              newShape = false;
              shapePath.c = false;
...
// import { registerModifier } from './ShapeModifiers';
var roundCorner$3 = 0.5519;

/**
 * a
 */

var RoundCornersModifier$2 = function (_ShapeModifier) {
  inherits(RoundCornersModifier, _ShapeModifier);
      ...
      for (var i = 0; i < len; i++) {
        ...
        if (currentV[0] === currentO[0] && currentV[1] === currentO[1] && currentV[0] === currentI[0] && currentV[1] === currentI[1]) {
          if ((i === 0 || i === len - 1) && !path.c) {
            ...
            vX = iX = currentV[0] + (closerV[0] - currentV[0]) * newPosPerc;
            vY = iY = currentV[1] - (currentV[1] - closerV[1]) * newPosPerc;
            oX = vX - (vX - currentV[0]) * roundCorner$3;
            oY = vY - (vY - currentV[1]) * roundCorner$3;
            ...
            vX = oX = currentV[0] + (closerV[0] - currentV[0]) * newPosPerc;
            vY = oY = currentV[1] + (closerV[1] - currentV[1]) * newPosPerc;
            iX = vX - (vX - currentV[0]) * roundCorner$3;
            iY = vY - (vY - currentV[1]) * roundCorner$3;
            cloned_path.setTripleAt(vX, vY, oX, oY, iX, iY, index);
            index += 1;
          }
        } else {
...
var RepeaterModifier$2 = function (_ShapeModifier) {
  inherits(RepeaterModifier, _ShapeModifier);
  ...
  createClass(RepeaterModifier, [{
    ...
    value: function initModifierProperties(elem, data) {
      ...
      this.so = PropertyFactory$1.getProp(elem, data.tr.so, 0, 0.01, this);
      this.eo = PropertyFactory$1.getProp(elem, data.tr.eo, 0, 0.01, this);
      this.data = data;
      if (!this.dynamicProperties.length) {
        this.getValue(true);
      }
      this._isAnimated = !!this.dynamicProperties.length;
      this.pMatrix = new Matrix$3();
      this.rMatrix = new Matrix$3();
      this.sMatrix = new Matrix$3();
      this.tMatrix = new Matrix$3();
...
var MouseModifier$2 = function (_ShapeModifier) {
  inherits(MouseModifier, _ShapeModifier);
 ...
 * @param {*} factory a
 */
function registerModifier$1(nm, factory) {
  if (!modifiers$1[nm]) {
    modifiers$1[nm] = factory;
  }
}
 ...
 * @return {*}
 */
function getModifier$1(nm, elem, data) {
  return new modifiers$1[nm](elem, data);
}

registerModifier$1('tm', TrimModifier$2);
registerModifier$1('rd', RoundCornersModifier$2);
registerModifier$1('rp', RepeaterModifier$2);
registerModifier$1('ms', MouseModifier$2);
 ...
 * @param {*} position a
 */
function ProcessedElement$2(element, position) {
  this.elem = element;
  this.pos = position;
}
...
var CVShapeElement = function () {
  ...
  function CVShapeElement(data) {
    ...
    this.transformsManager = new ShapeTransformManager$2();
    this.lcEnum = {
      '1': 'butt',
      '2': 'round',
      '3': 'square'
  ...
  createClass(CVShapeElement, [{
    ...
    value: function addProcessedElement(elem, pos) {
      ...
      elements.push(new ProcessedElement$2(elem, pos));
    }
    ...
    value: function createStyleElement(data, transforms) {
      ...
      var elementData = {};
      if (data.ty == 'fl' || data.ty == 'st') {
        elementData.c = PropertyFactory$1(this, data.c, 1, 255, this);
        if (!elementData.c.k) {
          styleElem.co = 'rgb(' + Math.floor(elementData.c.v[0]) + ',' + Math.floor(elementData.c.v[1]) + ',' + Math.floor(elementData.c.v[2]) + ')';
        }
      } else if (data.ty === 'gf' || data.ty === 'gs') {
        elementData.s = PropertyFactory$1(this, data.s, 1, null, this);
        elementData.e = PropertyFactory$1(this, data.e, 1, null, this);
        elementData.h = PropertyFactory$1(this, data.h || { k: 0 }, 0, 0.01, this);
        elementData.a = PropertyFactory$1(this, data.a || { k: 0 }, 0, degToRads$1, this);
        // elementData.g = new GradientProperty(this, data.g, this);
      }
      elementData.o = PropertyFactory$1(this, data.o, 0, 0.01, this);
      if (data.ty == 'st' || data.ty == 'gs') {
        styleElem.lc = this.lcEnum[data.lc] || 'round';
        styleElem.lj = this.ljEnum[data.lj] || 'round';
        if (data.lj == 1) {
        ...
        elementData.w = PropertyFactory$1(this, data.w, 0, null, this);
        if (!elementData.w.k) {
          styleElem.wi = elementData.w.v;
        }
        // if (data.d) {
    ...
    value: function createTransformElement(data) {
      return {
        transform: {
          ...
          _opMdf: false,
          key: this.transformsManager.getNewKey(),
          op: PropertyFactory$1(this, data.o, 0, 0.01, this),
    ...
    value: function searchShapes(arr, itemsData, prevViewData, shouldRender, transforms) {
      ...
      for (var i = len; i >= 0; i -= 1) {
        ...
        if (arr[i].ty == 'fl' || arr[i].ty == 'st' || arr[i].ty == 'gf' || arr[i].ty == 'gs') {
          ...
          if (!processedPos) {
            modifier = getModifier$1(arr[i].ty);
            modifier.init(this, arr[i]);
            itemsData[i] = modifier;
            this.shapeModifiers.push(modifier);
          } else {
...
var ShapeElement$2 = function (_Container) {
      ...
      for (var i = layers.length - 1; i >= 0; i--) {
        ...
        switch (layer.ty) {
          case 0:
            ...
            break;
          case 1:
            element = new SolidElement$2(layer, session);
            break;
          case 2:
            element = new SpriteElement$2(layer, session);
            break;
          case 3:
            element = new NullElement$2(layer, session);
            break;
          case 4:
            element = new ShapeElement$2(layer, session);
            break;
          default:
            continue;
        }
...
function completeLayers$1(layers, comps, fontManager) {
  var layerData;
  var i,
      len = layers.length;
  var j, jLen, k, kLen;
  for (i = 0; i < len; i += 1) {
    ...
    if (layerData.hasMask) {
      ...
      for (j = 0; j < jLen; j += 1) {
        if (maskProps[j].pt.k.i) {
          convertPathsToAbsoluteValues$1(maskProps[j].pt.k);
        } else {
          kLen = maskProps[j].pt.k.length;
          for (k = 0; k < kLen; k += 1) {
            if (maskProps[j].pt.k[k].s) {
              convertPathsToAbsoluteValues$1(maskProps[j].pt.k[k].s[0]);
            }
            if (maskProps[j].pt.k[k].e) {
              convertPathsToAbsoluteValues$1(maskProps[j].pt.k[k].e[0]);
            }
          }
        }
      }
    ...
    if (layerData.ty === 0) {
      layerData.layers = findCompLayers$1(layerData.refId, comps);
      completeLayers$1(layerData.layers, comps, fontManager);
    } else if (layerData.ty === 4) {
      completeShapes$1(layerData.shapes);
    } else if (layerData.ty == 5) {
      completeText$1(layerData, fontManager);
    }
  }
}

function findCompLayers$1(id, comps) {
  var i = 0,
      len = comps.length;
  while (i < len) {
    if (comps[i].id === id) {
...
function completeShapes$1(arr) {
  ...
  for (i = len - 1; i >= 0; i -= 1) {
    if (arr[i].ty == 'sh') {
      if (arr[i].ks.k.i) {
        convertPathsToAbsoluteValues$1(arr[i].ks.k);
      } else {
        jLen = arr[i].ks.k.length;
        for (j = 0; j < jLen; j += 1) {
          if (arr[i].ks.k[j].s) {
            convertPathsToAbsoluteValues$1(arr[i].ks.k[j].s[0]);
          }
          if (arr[i].ks.k[j].e) {
            convertPathsToAbsoluteValues$1(arr[i].ks.k[j].e[0]);
          }
        }
      }
      // hasPaths = true;
    } else if (arr[i].ty == 'gr') {
      completeShapes$1(arr[i].it);
    }
  }
  /*if(hasPaths){
      //mx: distance
...
function completeText$1(data) {
  if (data.t.a.length === 0 && !('m' in data.t.p)) {
    data.singleShape = true;
  }
}

function convertPathsToAbsoluteValues$1(path) {
  var i,
      len = path.i.length;
  for (i = 0; i < len; i += 1) {
    path.i[i][0] += path.v[i][0];
...
function checkVersion$1(minimum, animVersionString) {
  var animVersion = animVersionString ? animVersionString.split('.') : [100, 100, 100];
  if (minimum[0] > animVersion[0]) {
    return true;
  } else if (animVersion[0] > minimum[0]) {
...
var checkText$1 = function () {
  ...
  return function (animationData) {
    if (checkVersion$1(minimumVersion, animationData.v)) {
      iterateLayers(animationData.layers);
      if (animationData.assets) {
        var i,
...
var checkChars$1 = function () {
  var minimumVersion = [4, 7, 99];
  return function (animationData) {
    if (animationData.chars && !checkVersion$1(minimumVersion, animationData.v)) {
      var i,
      ...
      for (i = 0; i < len; i += 1) {
        if (animationData.chars[i].data && animationData.chars[i].data.shapes) {
          ...
          for (j = 0; j < jLen; j += 1) {
            pathData = paths[j].ks.k;
            if (!pathData.__converted) {
              convertPathsToAbsoluteValues$1(paths[j].ks.k);
              pathData.__converted = true;
            }
          }
        }
...
function completeData$1(animationData, fontManager) {
  if (animationData.__complete) {
    return;
  }
  checkColors$1(animationData);
  checkText$1(animationData);
  checkChars$1(animationData);
  checkShapes$1(animationData);
  completeLayers$1(animationData.layers, animationData.assets, fontManager);
  animationData.__complete = true;
  //blitAnimation(animationData, animationData.assets, fontManager);
}
...
var AnimationGroup$2 = function () {
  ...
  function AnimationGroup(options) {
    ...
    completeData$1(options.keyframes);
    ...
    this.register = new Register$1(this.keyframes.assets, this.prefix, crossOrigin);
    ...
    this.group = new CompElement$2(layers, {
      assets: assets,
      size: { w: w, h: h },
      prefix: this.prefix,
...
var AnimationManager$2 = function () {
  /**
  ...
  createClass(AnimationManager, [{
    ...
    key: 'parseAnimation',
    value: function parseAnimation(options) {
      var animate = new AnimationGroup$2(options);
      return this.add(animate);
    }
...
exports.AnimationManager = AnimationManager$2;
